...
import com.laplataenbici.model.repository.utils.query.BicicletaQuery;
...
public class BicicletaResource {
	...
	public Response getAll(@Context UriInfo uri) throws LPBException {
		return LPBResponse.ok(service.findAll(new BicicletaQuery(uri.getQueryParameters())));
	}
	...
	@Path("{id}/historial")
	@Secured
	public Response getHistorial(@Context UriInfo uri,@PathParam("id") Long id) throws LPBException {
		return LPBResponse.ok(historial.findAllFor(new BicicletaQuery(uri.getQueryParameters()),new Bicicleta(id)));
	}
